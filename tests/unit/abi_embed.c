...
typedef struct { AsyncHost host; int64_t got; int rc; const char *blob; } Task;
...
int main(void) {
  ...
  /* ---- 1. grant + host: an effect resolves to the host's value; over a store the same request is one host call ---- */
  {
    ...
    CHECK(h.calls == 1, "1: host reached %d times (want 1)", h.calls);
    CHECK(run1(b, slate_dag_add(b, fx(b, "req-1"), fx(b, "req-1")), &v) == 0 && v == 82 && h.calls == 1, "1: the same request reached the host again (%d)", h.calls);
    CHECK(run1(b, fx(b, "req-2"), &v) == 0 && v == 41 && h.calls == 2, "1: a distinct request did not reach the host (%d)", h.calls);
    slate_dag_free(b); st_free(&st);
    printf("  grant+host: effect reads the host's value (empirical); the same request is one call — its row; a distinct request a second\n");
  }
  ...
  /* ---- 2. the row outlives the builder: over a shared store a second builder is served without the host ---- */
  {
    ...
    long hits = st.hits; CHECK(run1(b2, fx(b2, "row"), &v) == 0 && v == 7 && h.calls == 1 && st.hits > hits, "2: the second builder reached the host (%d) / hit %ld", h.calls, st.hits - hits);
    ...
    CHECK(run1(b3, fx(b3, "row"), &v) != 0 && h.calls == 1, "2: an ungranted builder was served the row");
  ...
  /* ---- 3. fail-closed: no grant, the wrong grant, no host, the gate says no — the host is never reached ---- */
  {
    ...
    slate_dag_effect_caps(b, CAP_GENERIC, 1); slate_dag_effect_host(b, NULL, NULL, NULL); CHECK(run1(b, fx(b, "x"), &v) != 0, "3: no host did not refuse");
    slate_dag_effect_host(b, host_perform, NULL, &h); slate_dag_proceed(b, proceed_no, &asked);
    CHECK(run1(b, fx(b, "x"), &v) != 0 && asked > 0, "3: the deadline gate did not abort (asked %d)", asked);
    CHECK(h.calls == 0, "3: the host was reached %d times", h.calls);
    slate_dag_proceed(b, NULL, NULL); CHECK(run1(b, fx(b, "x"), &v) == 0 && v == 5 && h.calls == 1, "3: after clearing the gate");
    slate_dag_free(b);
    printf("  fail-closed: no grant / wrong grant / no host / gate says no — refused, host untouched; cleared, it fires\n");
  }
